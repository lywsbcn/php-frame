...
class Model {
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            // 如果数据表字段没有定义则自动获取
            if(_DC('fields_cache')) {
                $db   =  $this->dbName?$this->dbName:_DC('database');
    ...
    protected function _read_data($data) {
        // 检查字段映射
        if(!empty($this->_map) && _DC('READ_DATA_MAP')) {
     ...
     */
     public function create($data='',$type='') {
        ...
        if(isset($fields)) {
            ...
            // 判断令牌验证字段
            if(_DC('TOKEN_ON'))   $fields[] = _DC('TOKEN_NAME', null, '__hash__');
            foreach ($data as $key=>$val){
    ...
    public function autoCheckToken($data) {
        ...
        if(_DC('TOKEN_ON')){
            $name   = _DC('TOKEN_NAME', null, '__hash__');
            ...
            // 开启TOKEN重置
            if(_DC('TOKEN_RESET')) unset($_SESSION[$name][$key]);
            return false;
        }
        return true;
    }
    ...
    public function getModelName() {
        if(empty($this->name)){
            $name = substr(get_class($this),0,-strlen(_DC('DEFAULT_M_LAYER')));
